...
from lit_gpt.gentrans import GPTWithConv1D, Block, Config, adapter_filter, mark_only_adapter_as_trainable
...
train_data = torch.load(f'{data_dir}/train.pt')#, map_location=torch.device('cpu'))
val_data = torch.load(f'{data_dir}/dev.pt')#, map_location=torch.device('cpu'))
...
# Network stuff
max_input_length = 565  # 800 for v100 wo k,v ; 700 works for v100 w k,v
...
#run_name = f'{method}_{dataset}_{srclang}_{tgtlang}_{task}_{seamless_size}'  # added z at end to distinguish
...
def export_memory_snapshot() -> None:
    ...
    try:
        print(f"Saving snapshot to local file: {timestamp}.pickle")
        torch.cuda.memory._dump_snapshot(f"{timestamp}.pickle")
...
#    export_memory_snapshot()
    ...
    for iter_num in range(max_iters):
        ...
        input_ids, encoder_output, targets, speech_emb_len = get_batch(
            fabric, train_data, longest_seq_length, longest_seq_ix if iter_num == 0 else None
        )

        is_accumulating = (iter_num + 1) % gradient_accumulation_iters != 0
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            logits = model(input_ids, encoder_output, max_seq_length=max_seq_length + speech_emb_len, lm_head_chunk_size=128)
#            export_memory_snapshot()
...
@torch.no_grad()
def validate(
    fabric: L.Fabric, model: GPTWithConv1D, val_data: List[Dict], tokenizer: Tokenizer, longest_seq_length: int
) -> torch.Tensor:
    fabric.print("Validating ...")
    model.eval()
    losses = torch.zeros(eval_iters)
    for k in range(eval_iters):
        input_ids, encoder_output, targets, _ = get_batch(fabric, val_data, longest_seq_length)
        logits = model(input_ids, encoder_output)
...
def get_batch(
    ...
    encoder_output = [data[i]["encoder_output"][:max_input_length] for i in ix]
    ...
    max_len2 = max(s.shape[1] for s in encoder_output) if fabric.device.type != "xla" else longest_seq_length
    max_len2 = math.ceil(max_len2 / 5) * 5
    speech_emb_len = max_len2 // 5 
    max_len += speech_emb_len
    ...
    def pad_right_tensor(x):
        x_padded = torch.zeros(max_len2, x.shape[1])
        x_padded[:x.shape[0], :] = x
        return x_padded

    x = torch.stack([pad_right(x, pad_id=0) for x in input_ids])
    y = torch.stack([pad_right(x, pad_id=-1) for x in labels])
    x2 = torch.stack([pad_right_tensor(x.squeeze(0)) for x in encoder_output])

    if fabric.device.type == "cuda" and x.device.type == "cpu":
        x, x2, y = fabric.to_device((x.pin_memory(), x2.pin_memory(), y.pin_memory()))
    else:
        x, x2, y = fabric.to_device((x, x2, y))
    return x, x2, y, speech_emb_len
