...
from lit_gpt.gentrans import GPTWithQFormer, Block, Config, adapter_filter, mark_only_adapter_as_trainable
...
# Network stuff
max_input_length = 516  # 800 for v100 wo k,v ; 700 works for v100 w k,v
...
#    export_memory_snapshot()
    ...
    for iter_num in range(max_iters):
        ...
        input_ids, encoder_output, targets = get_batch(
            fabric, train_data, longest_seq_length, longest_seq_ix if iter_num == 0 else None
        )

        is_accumulating = (iter_num + 1) % gradient_accumulation_iters != 0
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            logits = model(input_ids, encoder_output, max_seq_length=max_seq_length+40, lm_head_chunk_size=128)
#            export_memory_snapshot()
...
@torch.no_grad()
def validate(
    fabric: L.Fabric, model: GPTWithQFormer, val_data: List[Dict], tokenizer: Tokenizer, longest_seq_length: int
) -> torch.Tensor:
    fabric.print("Validating ...")
    model.eval()
    losses = torch.zeros(eval_iters)
    for k in range(eval_iters):
        input_ids, encoder_output, targets = get_batch(fabric, val_data, longest_seq_length)
