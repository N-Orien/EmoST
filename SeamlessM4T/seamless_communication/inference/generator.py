...
from fairseq2.generation.sampling import TopPSampler
...
class UnitYGenerator:
    ...
    def __call__(
        ...
        n_best: bool = False,
    ) -> Tuple[List[StringLike], Optional[Tensor], Optional[Tensor]]:
        """
        :param source_seqs:
        ...
        if input_modality == "speech":
            texts, text_gen_output = self.s2t_converter.batch_convert(
                source_seqs, source_padding_mask, n_best=n_best
            )
        ...
        if output_modality == "text":
            return texts, None, text_gen_output.encoder_output
        ...
        return texts, units, text_gen_output.encoder_output
