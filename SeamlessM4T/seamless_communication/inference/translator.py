...
class Translator(nn.Module):
    ...
    def predict(
        ...
        decode_method: str = "beam",
        temperature: float = 0.0,
    ) -> Tuple[List[StringLike], Optional[BatchedSpeechOutput], Optional[Tensor]]:
        """
        ...
        texts, units, encoder_output = self.get_prediction(
            self.model,
            self.text_tokenizer,
            self.unit_tokenizer,
            seqs,
        ...
        if self.apply_mintox and task_str != Task.ASR.name:
            if input_modality == Modality.SPEECH:
                if src_text is not None:
                    ...
                    src_texts, _, encoder_output = self.predict(
                        input=input,
        ...
        if output_modality == Modality.TEXT:
            return texts, None, encoder_output
        else:
