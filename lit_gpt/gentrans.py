...
from lit_gpt.model import KVCache, apply_rope, build_rope_cache
#RoPECache
from lit_gpt.qformer import QFormer
from lit_gpt.conv1d import ProjectorConv1d
...
#        cos, sin = rope
        ...
        if self.block_idx >= self.config.adapter_start_layer:
            ...
            if adapter_kv_cache is not None:
                ...
                ak = ak.view(1, -1, aT, self.config.head_size)  # (1, nh_ak, aT, hs) = (1, 32, 10, 128)
...
#        rope: RoPECache,
        ...
        h, new_kv_cache, new_adapter_kv_cache = self.attn(
            n_1, cos, sin, max_seq_length, mask, input_pos, kv_cache, adapter_kv_cache
        )
...
class GPTWithQFormer(GPT):
    ...
    def forward(
        ...
        # forward the model itself
        x = self.transformer.wte(idx)  # token embeddings of shape (b, t, n_embd)
        speech_embedding = self.qformer(encoder_output)
        x = torch.cat([speech_embedding, x[:, 40:, :]], dim=1)
...
class GPTWithConv1D(GPT):
    ...
    def forward(
        ...
        # forward the model itself
        x = self.transformer.wte(idx)  # token embeddings of shape (b, t, n_embd)
        speech_embedding = self.projector(encoder_output)
        x = torch.cat([speech_embedding, x[:, speech_embedding.shape[1]:, :]], dim=1)
...
def adapter_filter(key: str, value: Any) -> bool:
    return "adapter_wte" in key or "gating_factor" in key or 'padding' in key or 'projection' in key or 'qformer' in key or 'projector' in key
