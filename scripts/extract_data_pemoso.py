...
def main():
    ...
    # Open output files
    with open(sentiment_path, 'w', encoding='utf-8') as sen_file, open(inferences_path, 'w', encoding='utf-8') as inf_file, open(ground_truths_path, 'w', encoding='utf-8') as gt_file:
        for item in data:
            if 'inference' in item and 'ground_truth' in item:
                # Write to inferences file
                sen_file.write(item['inference'][0] + '\n')
                inf_file.write(item['inference'][1] + '\n')
